...
static int32_t xensiv_pasco2_uart_read_sll1(const xensiv_pasco2_t * dev, uint8_t reg_addr, uint8_t * data, uint8_t len)
{
    ...
    for (uint8_t i = 0; i < len; ++i)
    {
        ...
        if (XENSIV_PASCO2_OK == res)
        {
            ...
            if (XENSIV_PASCO2_OK == res)
            {
                data[i] = (uint8_t)((xensiv_pasco2_ascii_to_digit(uart_buf[1]) << 4) + xensiv_pasco2_ascii_to_digit(uart_buf[2]));
            }
        }
...
int32_t xensiv_pasco2_init()
{
    ...
    if ((XENSIV_PASCO2_OK == res) && (XENSIV_PASCO2_COMM_TEST_VAL == data))
    {
        ...
        if (XENSIV_PASCO2_OK == res)
        {
            ...
            res = xensiv_pasco2_uart_read_sll1(&dev, (uint8_t)XENSIV_PASCO2_REG_SENS_STS, &data, 1U);
        }
...
int32_t xensiv_pasco2_get_measurement_config_sll1(const xensiv_pasco2_t * dev, xensiv_pasco2_measurement_config_t * meas_config)
{
    xensiv_pasco2_plat_assert(dev != NULL);
    xensiv_pasco2_plat_assert(meas_config != NULL);

    return xensiv_pasco2_uart_read_sll1(dev, (uint8_t)XENSIV_PASCO2_REG_MEAS_CFG, &(meas_config->u), 1U);
}
...
int32_t xensiv_pasco2_get_result(uint16_t * val)
{
    ...
    int32_t res = xensiv_pasco2_get_measurement_status_sll1(&dev, &meas_status);

    if (XENSIV_PASCO2_OK == res)
    {
        if (meas_status.b.drdy != 0U)
        {
            res = xensiv_pasco2_uart_read_sll1(&dev, (uint8_t)XENSIV_PASCO2_REG_CO2PPM_H, (uint8_t *)val, 2U);
